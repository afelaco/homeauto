...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry):
    ...
    entry.async_on_unload(hass.bus.async_listen_once(EVENT_HOMEASSISTANT_STOP, coordinator.async_stop))
...
class DahuaDataUpdateCoordinator(DataUpdateCoordinator):
    ...
    def __init__(
        ...
        self.dahua_vto_event_thread = DahuaVtoEventThread(
            hass, self.client, self.on_receive_vto_event, host=address, port=5000, username=username, password=password
        )
    ...
    def supports_infrared_light(self) -> bool:
        ...
        if not self._supports_lighting:
            return False
        return (
            "-AS-PV" not in self.model and "-AS-NI" not in self.model and "LED-S2" not in self.model
